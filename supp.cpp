                    ...
                    const double Ly)
{
    ...
    }else if(propagationFunctionName == "Case3"){
		return [=](const double x, const double y) -> double {
			/*
			// Read the default input
			string inputPathSupp = "supp.in";
			// Optionally override configuration file.
			if (argc > 1) inputPathSupp = argv[1];

			ConfigFile configFileSupp(inputPathSupp);
			// Override settings
			for (int i = 2; i < argc; i++)
				configFileSupp.process(argv[i]);

			const double xa = configFileSupp.get<double>("xa");
			const double ya = configFileSupp.get<double>("ya");
			const double Aa = configFileSupp.get<double>("Aa");
			const double s1a = configFileSupp.get<double>("s1a");
			const double s2a = configFileSupp.get<double>("s2a");
			
			const double xb = configFileSupp.get<double>("xb");
			const double yb = configFileSupp.get<double>("yb");
			const double Ab = configFileSupp.get<double>("Ab");
			const double s1b = configFileSupp.get<double>("s1b");
			const double s2b = configFileSupp.get<double>("s2b");
			
			const double B = configFileSupp.get<double>("B");
			const double px = configFileSupp.get<double>("px");
			const double py = configFileSupp.get<double>("py");
			*/
			//supplémentaire 1
			/*
			const double xa= 350;
			const double ya= 250;
			const double Aa= -2.00;
			const double s1a= 170;
			const double s2a=5;
			
			const double xb=200;
			const double yb=100;
			const double Ab=-1.75;
			const double s1b=20;
			const double s2b=150;
			

			const double B=5.00;
			const double px=0.001;
			const double py=0.005;
			*/
			...
			const double xa= 350;
			const double ya= 250;
			const double Aa= 4.25;
			const double s1a= 350;
			const double s2a=10;
			
			const double xb=150;
			const double yb=250;
			const double Ab=3.0;
			const double s1b=200;
			const double s2b=20;
			...
			double h=Aa*exp(-1/(s1a*s1a)*pow(y-ya,2.0)-1/(s2a*s2a)*pow(x-xa,2.0))+Ab*exp(-1/(s1b*s1b)*pow((y-yb),2.0)-1/(s2b*s2b)*pow(x-xb,2.0))-B+px*x+py*y;
			//double h=Aa*exp(-1/(s1a*s1a)*pow(y-ya,2.0)-1/(s2a*s2a)*pow(x-xa,2.0));
			//double h=Aa*exp(-1/(s1a*s1a)*pow(y-ya,2.0)-1/(s2a*s2a)*pow(x-xa,2.0))+Ab*exp(-1/(s1b*s1b)*pow((y-yb),2.0)-1/(s2b*s2b)*pow(x-xb,2.0));
                      ...
                      const WaveSpeedFunction u2)
{
	...
	if(boundaryConditions.right==BoundaryType::Exit){
		...
		for(unsigned int i(0);i<heightNext.back().size()-1;i++){
			...
			//heightNext[299][i]=height[299][i] -sqrt(u2(x.back(),y[i]))*dt/hx*(height[299][i]-height[298][i]);
		}
	}
                   ...
                   const WaveSpeedFunction u2)
{
    ...
    for (unsigned int i = 1; i < x.size() - 1; ++i) {
        for (unsigned int j = 1; j < y.size() - 1; ++j) {
             ...
             //heightNext[i][j] = u2(x[i],y[j])*dt*dt*((height[i+1][j]-2.0*height[i][j]+height[i-1][j])/hx2+(height[i][j+1]-2.0*height[i][j]+height[i][j-1])/hy2)
             //+2.0*height[i][j]-heightPrev[i][j];
        }
    }
}
